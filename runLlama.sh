...
if command -v nvidia-smi &> /dev/null && nvidia-smi &> /dev/null; then
    echo "🟢 GPU NVIDIA detectada! Ativando aceleração por hardware para o Llama.cpp."
    GPU_FLAG="--gpus all"
    HAS_GPU="true"
else
    echo "🟡 Nenhuma GPU detectada. O Llama.cpp rodará nativamente na CPU."
    GPU_FLAG=""
    HAS_GPU="false"
fi
...
echo "🎮 Executando Pipeline Llama.cpp + Unity via Script Interno Isolado..."
docker run --rm \
  $GPU_FLAG \
  --entrypoint /bin/bash \
  --shm-size=4gb \
